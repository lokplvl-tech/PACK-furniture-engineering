# Characteristics Registry: Мебельный инжиниринг

> Источник: технические справочники обоих источников (`06-sota/datum-mebel-epistemic-standard.md` §7, `06-sota/oneon-kitchen-tehchast.md` §4-§8).
> Регистр называет и категоризирует измеримые атрибуты домена — полные числовые таблицы остаются в источниках, здесь не дублируются (OwnerIntegrity).

---

## What is a Characteristic?

Per FPF (A.17 CHR-NORM): A **characteristic** is a measurable or observable attribute of the object of description within this domain. Characteristics describe **what** is measured or observed, not **how** to develop or change it.

---

## Characteristics Index

| ID | Characteristic | Category | Epistemic Stage |
|----|-----------------|----------|--------------------|
| [FE.CHR.001](#fe-chr-001) | Технический зазор | Геометрия/безопасность | evidence |
| [FE.CHR.002](#fe-chr-002) | Глубина корпуса модуля | Геометрия | evidence |
| [FE.CHR.003](#fe-chr-003) | Размер полки | Геометрия (производная) | evidence |
| [FE.CHR.004](#fe-chr-004) | Погонаж гарнитура | Коммерческая | evidence |
| [FE.CHR.005](#fe-chr-005) | Габарит встраиваемой техники | Совместимость | evidence |
| [FE.CHR.006](#fe-chr-006) | Нагрузочная способность направляющих | Фурнитура | evidence |
| [FE.CHR.007](#fe-chr-007) | Класс эмиссии формальдегида плитных материалов | Материал/безопасность | evidence |
| [FE.CHR.008](#fe-chr-008) | Размер и термостойкость столешницы | Геометрия/материал | evidence |
| [FE.CHR.009](#fe-chr-009) | Точность присадочных отверстий | Технологический допуск | evidence |
| [FE.CHR.010](#fe-chr-010) | Зазор по высоте вытяжки над варочной поверхностью | Безопасность/термодинамика | evidence |
| [FE.CHR.011](#fe-chr-011) | Эргономическая ширина прохода между рядами | Эргономика | evidence |

---

## [FE.CHR.001] Технический зазор

**Definition**: Минимально допустимое расстояние между корпусом модуля и физическим препятствием (стеной, трубой, соседним прибором), обеспечивающее циркуляцию воздуха, монтажный допуск или пожарную безопасность.

**Category**: Геометрия/безопасность

**Indicators**:
- От стены (с техникой / без техники): 51 мм / 41 мм
- От трубы отопления: 35 мм
- От метровика (по боковине / по оргалиту): 45 мм / 55 мм
- От плиты/варочной панели до пенала/мойки/холодильника: 150 мм
- Полный справочник значений — `06-sota/datum-mebel-epistemic-standard.md` Таблица 7.1

**Measurement/test methods**:
- [FE.M.001](../03-methods/FE.M.001.md) — фиксируется на шагах 2a, 3a при расстановке
- Отчёт DRR ([FE.WP.001](../04-work-products/FE.WP.001.md)) — контрольный артефакт

**Related distinctions**:
- [FE.D.006](../01-domain-contract/01B-distinctions.md#fe-d-006) Инвариант vs Настраиваемый параметр — зазор относится к инварианту, не выбирается произвольно

**Related failure modes**:
- [FE.GRD.001](../05-failure-modes/FE.GRD.001.md), [FE.GRD.002](../05-failure-modes/FE.GRD.002.md)

**Epistemic stage**: `evidence` — числа совпадают дословно в двух независимых источниках (Датум Мебель, ONEON Kitchen)

---

## [FE.CHR.002] Глубина корпуса модуля

**Definition**: Стандартная глубина корпуса модуля (без фасада), зависящая от типа модуля и его функции.

**Category**: Геометрия

**Indicators**:
- Нижний модуль: 553 мм по корпусу (~600 мм габарит с фасадом и ручкой)
- Верхний модуль: 300 мм по корпусу (328 мм с фасадом)
- Пенал: 598 мм (604 мм под холодильник)
- Метровик (угловой): 555×955 мм

**Measurement/test methods**:
- [FE.M.001](../03-methods/FE.M.001.md) — задаётся выбором типа модуля на шагах 2a-7

**Related distinctions**:
- [FE.D.008](../01-domain-contract/01B-distinctions.md#fe-d-008) Холон vs Модуль

**Related objects of attention**:
- [FE.OA.001](02B-objects-of-attention.md#fe-oa-001) Нижний модуль, [FE.OA.002](02B-objects-of-attention.md#fe-oa-002) Верхний модуль, [FE.OA.003](02B-objects-of-attention.md#fe-oa-003) Пенал, [FE.OA.004](02B-objects-of-attention.md#fe-oa-004) Угловой модуль

**Epistemic stage**: `evidence` — совпадает дословно в обоих источниках (пеналы — идентичные числа для высот 1420/2140/2340)

---

## [FE.CHR.003] Размер полки

**Definition**: Ширина и глубина съёмной полки внутри модуля — производная характеристика, вычисляемая из габарита модуля по фиксированной формуле, а не выбираемая напрямую.

**Category**: Геометрия (производная)

**Indicators**:
- Ширина = ширина модуля − 32 мм (толщины боковин) − 2 мм (зазор)
- Глубина = глубина модуля − 10 мм
- Результирующая глубина: 270 мм (верхние модули), 495 мм (нижние), 540 мм (пеналы)
- Полка в модуле с газлифтом: строго 240 мм (укорочена под механизм)

**Measurement/test methods**:
- Расчёт при заказе комплектующих — часть шага 8 [FE.M.001](../03-methods/FE.M.001.md)

**Related distinctions**:
- [FE.D.006](../01-domain-contract/01B-distinctions.md#fe-d-006) Инвариант vs Настраиваемый параметр — формула сама инвариант, но её входные габариты — настраиваемый параметр

**Epistemic stage**: `evidence` — формула дословно совпадает в обоих источниках, включая частный случай газлифта

---

## [FE.CHR.004] Погонаж гарнитура

**Definition**: Взвешенная длина кухонного гарнитура, к которой применяется ценовой коэффициент — не сумма всех линейных размеров, а сумма с разным весом для нижнего и верхнего яруса.

**Category**: Коммерческая

**Indicators**:
- Формула: Σ(нижние модули) × 0,7 + Σ(верхние модули) × 0,3
- Угловые модули считаются по обеим сторонам прилегания к стене
- Порог полноценного гарнитура: от 1100 мм

**Measurement/test methods**:
- [FE.M.003](../03-methods/FE.M.003.md) — единственный метод, вычисляющий эту характеристику

**Related distinctions**:
- [FE.D.006](../01-domain-contract/01B-distinctions.md#fe-d-006) — коэффициенты 0,7/0,3 сами инвариант формулы

**Related roles**:
- [FE.R.006](02A-roles.md#fe-r-006) Менеджер

**Epistemic stage**: `evidence` — коэффициенты 70%/30% кросс-валидированы дословно двумя компаниями (см. `06-sota/furniture-engineering-sota-sheet.md`)

---

## [FE.CHR.005] Габарит встраиваемой техники

**Definition**: Стандартизированные размеры бытовой техники (ширина/высота/глубина), определяющие размер модуля и фасада, в который она встраивается.

**Category**: Совместимость

**Indicators**:
- Варочная панель: 600 мм (4 конфорки) / 450 мм (3) / 300 мм (2)
- Духовой шкаф: 600×600 стандарт, 600×450 узкий, 450×600 компактный
- ПММ встроенная: 450 мм (8-9 комплектов) / 600 мм (12-13 комплектов)
- Стиральная машина встроенная: 600 мм, фасад 717×597
- Полный справочник — `06-sota/oneon-kitchen-tehchast.md` §6

**Measurement/test methods**:
- [FE.M.001](../03-methods/FE.M.001.md) — интеграция техники на шаге 4; для встроенной техники обязателен точный артикул

**Related failure modes**:
- [FE.GRD.006](../05-failure-modes/FE.GRD.006.md) — расчёт без артикула

**Epistemic stage**: `evidence` — базовые типоразмеры (варочная панель, духовой шкаф) совпадают в обоих источниках; точный фасад по артикулу — специфика конкретной модели техники, не домена

---

## [FE.CHR.006] Нагрузочная способность направляющих

**Definition**: Максимальная масса, которую выдерживает механизм выдвижения ящика без деформации или отказа — определяет допустимую область применения конкретного типа направляющих.

**Category**: Фурнитура

**Indicators**:
- ПВШ (полно-выкатные шариковые): до 20 кг (рабочее значение обоих источников; ONEON §2 даёт общий диапазон по рынку 15-20 кг, а конкретная бундлируемая фурнитура §4 — 20 кг, как и у Датум Мебель — расхождения по сути нет, это верхняя граница диапазона) — только в ящике 120 мм под духовым шкафом
- Тандембокс BOYARD: 30-50 кг в зависимости от модели
- Тандембокс Hettich Attira: до 50 кг
- Тандембокс Blum: до 50 кг

**Measurement/test methods**:
- Выбор фурнитуры на шаге 8 [FE.M.001](../03-methods/FE.M.001.md), после продажи ([FE.R.006](02A-roles.md#fe-r-006) Менеджер)

**Related distinctions**:
- [FE.D.007](../01-domain-contract/01B-distinctions.md#fe-d-007) Тандембокс vs ПВШ — прямое следствие разницы в этой характеристике

**Epistemic stage**: `evidence` — граница применимости ПВШ (только под духовым шкафом, узкая ниша по нагрузке) совпадает в обоих источниках

**Уточнение (2026-07-26), разрешено:** синтезированный источник (`06-sota/kitchen-systems-engineering-playbook.md`, обе версии) уточняет границу — «ПВШ для шкафов-купе, в кухнях предпочтительны скрытые направляющие под дном ящика».

Не строгий запрет (не противоречит нише «только под духовым шкафом» из ONEON §4), а явное предпочтение для основного объёма кухонных ящиков — согласуется с узостью ниши ПВШ в кухонном контексте.

---

## [FE.CHR.007] Класс эмиссии формальдегида плитных материалов

**Definition**: Категория, характеризующая безопасность плитного материала (ЛДСП/МДФ) по уровню выделения формальдегида — определяет допустимость применения в жилых помещениях.

**Category**: Материал/безопасность

**Indicators**:
- Класс Е1: до 10 мг на 100 г сухой плиты — стандарт для жилых помещений
- Класс Е2: 10-30 мг на 100 г — упомянут у ONEON как более низкий класс
- ГОСТ 10632-89 — регламентирующий стандарт, явно назван у ONEON; у Датум тот же порядок цифр указан без номера ГОСТа

**Measurement/test methods**:
- Сертификат производителя плиты (EGGER, Увадрев)

**Related distinctions**: —

**Epistemic stage**: `evidence` — числа Е1/до 10 мг совпадают дословно в обоих источниках; номер ГОСТа — уточнение из ONEON, не расхождение

---

## [FE.CHR.008] Размер и термостойкость столешницы

**Definition**: Геометрические и термические характеристики столешницы (постформинг: ДСП-основа + HPL-облицовка), определяющие совместимость с врезной техникой и допустимые тепловые нагрузки.

**Category**: Геометрия/материал

**Indicators**:
- Толщина: 38 мм (стандарт для большинства врезной техники)
- Термостойкость: до 180°C
- Длина: фактическая 3050 мм / рабочая 3000 мм (стандарт); нестандартная — до 4250 мм (Скиф) или 4050 мм (Кедр), срок изготовления нестандартной 30 дней
- Глубина: 600 мм
- Еврозапил: к габариту столешницы добавляется +50 мм

**Measurement/test methods**:
- [FE.M.001](../03-methods/FE.M.001.md) — шаг 3a (столешница)

**Related distinctions**:
- [FE.D.006](../01-domain-contract/01B-distinctions.md#fe-d-006) Инвариант vs Настраиваемый параметр — толщина и термостойкость инвариантны, длина/цвет настраиваемы

**Related failure modes**:
- [FE.GRD.008](../05-failure-modes/FE.GRD.008.md) — врезка мойки в зону стыка столешниц

**Epistemic stage**: `evidence` — толщина 38 мм, +50 мм еврозапил, глубина 600 мм совпадают дословно в обоих источниках; термостойкость 180°C указана только у Датум (дополнение, не расхождение)

---

## [FE.CHR.009] Точность присадочных отверстий

**Definition**: Максимально допустимое отклонение от номинального положения отверстия под крепёж или фурнитуру при сверлении на присадочном станке.

**Category**: Технологический допуск

**Indicators**:
- Максимальная погрешность присадки: 1 мм
- Превышение погрешности приводит к непопаданию евровинта в ответное отверстие; повторная закрутка крошит плиту ДСП (конфирмант держит только при первичном вкручивании)

**Measurement/test methods**:
- [FE.M.002](../03-methods/FE.M.002.md) — стадия 3 (Присадка)

**Related roles**:
- [FE.R.005](02A-roles.md#fe-r-005) Фабрика

**Epistemic stage**: `evidence` — значение 1 мм совпадает дословно в обоих источниках

---

## [FE.CHR.010] Зазор по высоте вытяжки над варочной поверхностью

**Definition**: Минимальное вертикальное расстояние между варочной панелью/плитой и нижним краем вытяжки — зависит от типа варочной поверхности (газ/электро) и типа вытяжки (встроенная/отдельная).

**Category**: Безопасность/термодинамика

**Indicators**:
- Встроенная вытяжка, газовая панель: 750 мм
- Встроенная вытяжка, электро/индукция: 560 мм
- Отдельно стоящая вытяжка, газовая панель: 750-850 мм
- Отдельно стоящая вытяжка, электро/индукция: 560-750 мм

**Measurement/test methods**:
- [FE.M.001](../03-methods/FE.M.001.md) — простановка на эскизе (обязательные размеры)

**Related distinctions**:
- [FE.D.006](../01-domain-contract/01B-distinctions.md#fe-d-006) Инвариант vs Настраиваемый параметр

**Epistemic stage**: `evidence` — все четыре значения совпадают дословно в обоих источниках

---

## [FE.CHR.011] Эргономическая ширина прохода между рядами

**Definition**: Минимальное расстояние между противоположными рядами модулей в П-образной или островной планировке, обеспечивающее свободный проход и одновременное открывание фасадов/ящиков с обеих сторон.

**Category**: Эргономика

**Indicators**:
- Рекомендуемое расстояние: 900 мм

**Measurement/test methods**:
- Простановка на эскизе при П-образной/островной планировке; при несоблюдении — обязательная предупреждающая метка в CAD

**Related distinctions**: —

**Epistemic stage**: `evidence` — значение 900 мм совпадает дословно в обоих источниках

**Расхождение найдено веб-поиском (2026-07-27), не разрешено:** вторичные дизайнерские источники (не ГОСТ, не первичная практика компаний) называют диапазон 1200-1650 мм для прохода между рядами — заметно шире 900 мм.

Не переписываю значение `evidence`-уровня (кросс-валидировано двумя компаниями) второсортным источником — фиксирую расхождение для пилота.

Возможно, 900 мм — минимально допустимое (для однорядной/узкой кухни), а 1200-1650 мм — рекомендация для комфортного прохода при двухстороннем открывании фасадов с обеих сторон одновременно (другое практическое условие, не то же самое измерение).

Требует уточнения у пилота или сверки первичного текста источника, не силами одного веб-поиска.

---

_Characteristics per FPF A.17 CHR-NORM, SPF.SPEC.001/003. Pack ID: FE._
