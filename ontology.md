...
| P2W | Принципы в работу | Principles to Work | Pack — код совпадает с паттерном FPF `E.18.1` («P2W Problem-to-Work Carry-Through»), но это омоним, не то же понятие: здесь P2W — собственное название метода компании «Датум Мебель» (см. `06-sota/`), не редакция официального паттерна. **Не путать: FPF-паттерн `E.18.1` расшифровывается как «Проблема в работу» (Problem-to-Work), метод FE.M.001 этого пака — «Принципы в работу» (Principles to Work).** Переименование не проводится, чтобы не приписывать источнику чужую формулировку |
